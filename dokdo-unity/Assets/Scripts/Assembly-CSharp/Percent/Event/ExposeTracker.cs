...
{
	public class ExposeTracker : InHouseTracker
	{
		protected override string getUrl(PromotionType type, int clickedGameId)
		{
			if (!UUIDLoader.hasUUID())
			{
				Logger.error("Can NOT send expose event. UUID is NOT exist.");
				return string.Empty;
			}
			...
			HttpsClient.addResource(ref result, PercentHttpConfig.REQ_RESOURCE_EXPOSE);
