...
public class SampleTimer : MonoBehaviour
{
	...
	private void OnGUI()
	{
		...
		{
			text = string.Format("{0}:{1:D2}:{2:D2}", timeSpan.Hours, timeSpan.Minutes, timeSpan.Seconds);
		}
		...
		{
			text2 = string.Format("{0}:{1:D2}:{2:D2}", timeSpan2.Hours, timeSpan2.Minutes, timeSpan2.Seconds);
		}
