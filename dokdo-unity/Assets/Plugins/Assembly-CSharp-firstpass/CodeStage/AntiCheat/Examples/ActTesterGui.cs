...
namespace CodeStage.AntiCheat.Examples
{
	...
	public class ActTesterGui : MonoBehaviour
	{
		...
		private void OnGUI()
		{
			...
			else
			{
				...
				using (new VerticalLayout(GUI.skin.box))
				{
					...
					GUILayout.Label("<color=\"" + ((!timeCheatingDetected) ? "#02C85F" : "#FF4040") + "\">Detected: " + timeCheatingDetected.ToString().ToLower() + "</color>");
					GUILayout.Space(10f);
		...
		private string GetAllSimpleObscuredTypes()
		{
			...
			{
				Assembly assembly = AppDomain.CurrentDomain.GetAssemblies().SingleOrDefault((Assembly assembly2) => assembly2.GetName().Name == "Assembly-CSharp-firstpass");
				if (assembly != null)
				{
